...
def taxa_from_tips(tips_path: Path) -> list[str]:
    taxa = []
    with tips_path.open(encoding="utf-8") as handle:
        for line in handle:
            tip = line.strip()
            if not tip:
                continue
            taxa.append(tip if tip.endswith(".fa") else f"{tip}.fa")
    if not taxa:
        raise SystemExit(f"No taxa found in tips file: {tips_path}")
    return taxa
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build CRC/control sample CC_TCG DiTASiC abundance matrices in wide TSV format."
    )
    parser.add_argument(
        "--tips",
        default="/path/to/crc-metagenome-mag-pipeline/gtdbtk_denovo/tips.txt",
        help="CC_TCG tips.txt used for row order.",
    )
    parser.add_argument(
        "--data_root",
    ...
    parser.add_argument(
        "--out_dir",
        default="/path/to/crc-metagenome-mag-pipeline/gtdbtk_denovo",
        help="Output directory for *_CRC_CC_TCG_abundance_matrix.tsv files.",
    )
    parser.add_argument(
        "--datasets",
        nargs="+",
    ...
    taxa = taxa_from_tips(Path(args.tips))
    ...
    for dataset in args.datasets:
        ...
        out_path = out_dir / f"{dataset}_CRC_CC_TCG_abundance_matrix.tsv"
        stats = write_matrix(out_path, taxa, samples, abundance_dir)
        summary_rows.append((dataset, out_path, stats))

    for dataset, out_path, stats in summary_rows:
        print(
